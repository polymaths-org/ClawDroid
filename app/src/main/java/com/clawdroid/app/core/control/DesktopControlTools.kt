...
/**
 * Desktop (Interpole) control: mouse, clicks, drag, scroll, keys and screenshots
 * on a paired desktop — Linux (Hyprland / Wayland / X11), Debian and Windows.
 *
 * Transport: commands run locally when no host is configured (demo mode — e.g. the
 * Arch Hyprland machine itself), otherwise wrapped in `ssh -p PORT HOST 'cmd'`
 * (key-based auth, BatchMode so it never blocks on a password prompt).
 *
 * Gating: everything no-ops with `interpole_disabled` unless the master switch
 * ([InterpoleConfig]) is ON. Hyprland easy-mode picks hyprctl/ydotool/grim paths.
 */
object DesktopControlTools {
    ...
    fun resolveTarget(explicit: String?): OsTarget {
        ...
        return if (InterpoleConfig.isHyprlandEasy()) OsTarget.HYPRLAND else OsTarget.WAYLAND
    }
    ...
    suspend fun screenshot(context: Context, os: String?): JSONObject = runTool {
        guard() ?: run {
            ...
            val remotePath = "/tmp/interpole-$stamp.png"
            val result = runShell(context, screenshotCmd(t, remotePath), 30)
            if (result.exitCode != 0) {
                return@run errorResult(
                    "capture_failed",
                    "Screenshot command failed: ${result.output.take(300)}",
                )
            }
            val localPath = fetchScreenshot(context, remotePath, stamp)
                ?: return@run errorResult(
                    "fetch_failed",
                    "Captured to $remotePath but could not copy it to this device.",
                )
            JSONObject()
                .put("success", true)
    ...
    private fun sshPrefix(): String? {
        val host = com.clawdroid.app.core.config.AppConfigManager.interpoleHost.trim()
        if (host.isEmpty()) return null
        val port = com.clawdroid.app.core.config.AppConfigManager.interpolePort
        return "ssh -o BatchMode=yes -o ConnectTimeout=8 -p $port $host "
    }

    private suspend fun runShell(context: Context, command: String, timeoutSeconds: Long): com.clawdroid.app.core.tools.CommandResult {
        val prefix = sshPrefix()
        val full = if (prefix == null) {
            command
        } else {
            prefix + shQuote(command)
        }
        return CommandTool.execute(context, full, cwd = null, timeoutSeconds = timeoutSeconds)
    }
    ...
    private suspend fun execOk(
        ...
        val result = runShell(context, command, 20)
        if (result.exitCode != 0) {
            return errorResult(
    ...
    /** Copy a target-side screenshot onto this device (Output folder preferred). */
    private suspend fun fetchScreenshot(context: Context, remotePath: String, stamp: Long): String? {
        ...
        return runCatching {
            val prefix = sshPrefix()
            if (prefix == null) {
                File(remotePath).takeIf { it.exists() }?.copyTo(dest, overwrite = true)
            } else {
                val host = com.clawdroid.app.core.config.AppConfigManager.interpoleHost.trim()
                val port = com.clawdroid.app.core.config.AppConfigManager.interpolePort
                val cp = CommandTool.execute(
                    context,
                    "scp -o BatchMode=yes -o ConnectTimeout=8 -P $port $host:${shQuote(remotePath)} ${shQuote(dest.absolutePath)}",
                    cwd = null,
                    timeoutSeconds = 30,
                )
                if (cp.exitCode != 0 || !dest.exists()) return null
            }
    ...
    private fun guard(): JSONObject? {
        val err = InterpoleConfig.guardOrError() ?: return null
        return errorResult("interpole_disabled", err)
    }
